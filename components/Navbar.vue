<template>
    <nav class="navbar_main ">
      <div class="mobile_menu" :class="{'show':isOpen}">
        <ul class="nav">
      ...
      <div class="navbar_bottom_border">
        ...
        <div class="nav_links">
          <ul class="nav">
            <li class="nav-item" v-for="(item,index) in menu" :key="index">
            <NuxtLink :to="localePath(`/${item.url}`)" class="nav-link">{{item.title}}  </NuxtLink>
              <ul class="child_menu z_index1" v-if="item.child.length!=0">
                <li v-for="(it,ind) in item.child " :key="ind">
                  <NuxtLink :to="localePath(`/${it.url}`)"  >{{it.title}}</NuxtLink>
                </li>
              </ul>
          ...
          <div class="main_language">
            <button @click="isActive=!isActive">
              ...
              <span v-if="$i18n.locale=='uz'">O'zbek</span>
              <span v-if="$i18n.locale=='ru'">Рус</span>
          ...
          <div class="hambur">
            <div id="nav-icon2" :class="{'open':isOpen}" @click="isOpen=!isOpen">
            <span></span>
            <span></span>
            <span></span>
            <span></span>
